Shader "FluidSim/Jacobi" 
{
	SubShader 
	{
    	Pass 
    	{
			...
			{
			    float4 pressure_center = tex2D(_Pressure, IN.uv);

				float4x4 cardinals = GetCardinal(IN.uv, _InverseSize, _Obstacles, _Pressure, pressure_center);
			
			    float diverge = tex2D(_Divergence, IN.uv).x;
			    
			    return (cardinals._11 + cardinals._21 + cardinals._31 + cardinals._41 + _Alpha * diverge) * _InverseBeta;
